...
def top_3_words(text):
    my_string_start = sub(r'^\W+', '', text)
    mystring = sub(r'([^\w\s]|_)+(?=\s|$)', '', my_string_start)

    words = [word for word in mystring.lower().split()]

    d = {k: v for k, v in sorted(Counter(words).items(), key=lambda item: item[1], reverse=True)}
    first3pairs = {k: d[k] for k in list(d)[:3]}

    return list(first3pairs.keys())
